...
void rpc_send_lease_stats (struct connection *c) {
  ...
  *(double *)(q + qn) = lease_stats_time;
  ...
  send_rpc_query (c, TL_MLANG_LEASE_STATS, -1, q, qn * 4);
}
...
#define MAX_RPC_QUERY_LEN 126214400
  ...
  switch (op) {
  case TL_MLANG_STOP_LEASE:
    do_rpc_stop_lease();
    break;
  case TL_MLANG_START_LEASE:
  case RPC_INVOKE_MLANG_REQ:
  case RPC_INVOKE_REQ:
    ...
    vkprintf (2, "got RPC_INVOKE_MLANG_REQ [req_id = %016llx]\n", req_id);
    set_connection_timeout (c, script_timeout);
