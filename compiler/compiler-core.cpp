...
int scan_dir_callback (const char *fpath, const struct stat *sb, int typeflag) {
  if (typeflag == FTW_D) {
    ...
    mlang_error (0, dl_pstr ("Failed to scan directory [fpath=%s]\n", fpath));
    mlang_fail();
  }
  return 0;
}
...
void Index::del_extra_files() {
  ...
  for (int i = 0; i < (int)files.size(); i++) {
    if (!files[i].needed) {
      ...
      if ((n >= 2 && path[n - 2] == '.' && path[n - 1] == 'h') ||
        ...
        mlang_error (0, dl_pstr ("Unexpected file [%s] in destination folder is found", path.c_str()));
        return;
      }
    }
  }
  for (int i = 0; i < (int)files.size(); i++) {
    if (files[i].needed) {
      ...
      if (err != 0) {
        mlang_error (0, "Failed to unlink file");
        mlang_fail();
      }
    }
  }
...
bool CompilerCore::add_to_function_set (FunctionSetPtr function_set, FunctionPtr function,
  ...
  if (req) {
    mlang_assert (function_set->size() == 0);
...
FunctionSetPtr CompilerCore::get_function_set (function_set_t type, const string &name, bool force) {
  ...
  mlang_assert (function_set->name == name/*, "Bug in compiler: hash collision"*/);
  return function_set;
}
...
VarPtr CompilerCore::get_global_var (const string &name, VarData::Type type,
  ...
  if (new_var.is_null()) {
    mlang_assert (var->name == name/*, "bug in compiler (hash collision)"*/);
    if (init_val.not_null() && init_val->type() == op_string) {
      mlang_assert (var->init_val->type() == op_string);
      mlang_assert (var->init_val->get_string() == init_val->get_string());
    }
  }
  return var;
}
...
void CompilerCore::save_index() {
  ...
  mlang_error (err == 0, "Failed to rewrite index");
  mlang_fail();
}
...
VertexPtr set_func_id (VertexPtr call, FunctionPtr func) {
  mlang_assert (call->type() == op_func_ptr || call->type() == op_func_call);
  mlang_assert (func.not_null());
  mlang_assert (call->get_func_id().is_null() || call->get_func_id() == func);
  ...
  for (int i = 0; i < call_args_n; i++) {
    if (i < func_args_n) {
      if (func_args[i]->type() == op_func_param) {
        ...
        mlang_error (call_args[i]->type() == op_func_ptr, "Function pointer expected");
      } else {
        mlang_fail();
      }
    }
...
VertexPtr try_set_func_id (VertexPtr call) {
  ...
  mlang_assert (function_set->is_required);
