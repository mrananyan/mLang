...
void GenTree::init (const vector <Token *> *tokens_new, GenTreeCallbackBase *callback_new) {
  ...
  mlang_assert (cur != end);
  end--;
  mlang_assert ((*end)->type() == tok_end);
...
#define expect(tp, msg) ({ \
  bool res;\
  if (mlang_error (test_expect (tp), dl_pstr ("Expected %s, found '%s'", msg, cur == end ? "END OF FILE" : (*cur)->to_str().c_str()))) {\
    res = false;\
  } else {\
    next_cur();\
    res = true;\
...
#define expect2(tp1, tp2, msg) ({ \
  mlang_error (test_expect (tp1) || test_expect (tp2), dl_pstr ("Expected %s, found '%s'", msg, cur == end ? "END OF FILE" : (*cur)->to_str().c_str())); \
  if (cur != end) {next_cur();} \
  1;\
})
...
VertexPtr GenTree::get_var_name_ref() {
  ...
  if (name.not_null()) {
    ...
    mlang_error (ref_flag == 0, "Expected var name");
  }
  return name;
}
  ...
  while (next_delim) {
    ...
    if (v.is_null()) {
      if (EmptyOp != op_err && (prev_delim || next_delim)) {
        ...
        mlang_error (0, "Expected something after ','");
        return false;
      } else {
        break;
      }
...
template <Operation Op> VertexPtr GenTree::get_conv() {
  ...
  CE (!mlang_error (first_node.not_null(), "get_conv failed"));
...
template <Operation Op> VertexPtr GenTree::get_varg_call() {
  ...
  CE (!mlang_error (ok_args_next, "get_varg_call failed"));
...
template <Operation Op> VertexPtr GenTree::get_require() {
  ...
  CE (!mlang_error (ok_next, "get_require_list failed"));
...
template <Operation Op, Operation EmptyOp> VertexPtr GenTree::get_func_call() {
  ...
  CE (!mlang_error (ok_next, "get_reqire failed"));
  CE (expect (tok_clpar, "')'"));
...
VertexPtr GenTree::get_string_build() {
  ...
  while (cur != end && (*cur)->type() != tok_str_end) {
    if ((*cur)->type() == tok_str) {
      ...
      CE (!mlang_error (add.not_null(), "Bad expression in string"));
...
VertexPtr GenTree::get_postfix_expression (VertexPtr res) {
  ...
  while (need && cur != end) {
    ...
    if (tp == tok_inc) {
      ...
      CE (!mlang_error (i.not_null(), "Failed to parse right argument of '->'"));
...
VertexPtr GenTree::get_expr_top() {
  ...
  switch (type) {
    ...
    case tok_print: {
      ...
      CE (!mlang_error (first_node.not_null(), "Failed to get print argument"));
    ...
    case tok_oppar:
      next_cur();
      res = get_expression();
      CE (!mlang_error (res.not_null(), "Failed to parse expression after '('"));
...
VertexPtr GenTree::get_binary_op (int bin_op_cur, int bin_op_end, GetFunc next, bool till_ternary) {
  ...
  while (need && cur != end) {
    ...
    if (right.is_null()) {
      mlang_error (0, dl_pstr ("Failed to parse second argument in [%s]", OpInfo::str (tp).c_str()));
      return VertexPtr();
    }
    ...
    if (ternary) {
      ...
      if (third.is_null()) {
        mlang_error (0, dl_pstr ("Failed to parse third argument in [%s]", OpInfo::str (tp).c_str()));
        return VertexPtr();
      }
      left = conv_to <tp_bool> (left);
    }
...
VertexPtr GenTree::get_def_value() {
  ...
  if ((*cur)->type() == tok_eq1) {
    next_cur();
    val = get_expression();
    mlang_error (val.not_null(), "Cannot parse function parameter");
  }
...
VertexPtr GenTree::get_foreach_param () {
  ...
  CE (!mlang_error (xs.not_null(), ""));

  CE (expect (tok_as, "'as'"));

  VertexPtr x, key;
  x = get_var_name_ref();
  CE (!mlang_error (x.not_null(), ""));
  ...
  if (ref_flag) {
    switch (tp) {
      ...
      case tp_var:
        ...
        break;
      default:
        mlang_error (0, "convert_to not array with ref_flag");
        return x;
    }
  }
  switch (tp) {
...
PrimitiveType GenTree::get_type_help (void) {
  ...
  if ((*cur)->type() == tok_triple_colon) {
    next_cur();
    res = get_ptype();
    mlang_error (res != tp_Error, "Cannot parse type");
  }
  return res;
}
...
VertexPtr GenTree::get_type_rule_func (void) {
  ...
  CE (!mlang_error (ok_next, "Failed get_type_rule_func"));
  CE (expect (tok_gt, ">"));
...
VertexPtr GenTree::get_type_rule_ (void) {
  ...
  if (tp != tp_Error) {
    ...
    if (tp == tp_array && tok == tok_lt) {
      ...
      if (mlang_error (first.not_null(), "Cannot parse type_rule (1)")) {
        return VertexPtr();
      }
      CE (expect (tok_gt, "'>'"));
    }
    ...
    next_cur();
    if (mlang_error (test_expect (tok_int_const), "Int expected")) {
      return VertexPtr();
    }
    int cnt = 0;
...
VertexPtr GenTree::get_multi_call (GetFunc f) {
  ...
  CE (!mlang_error (ok_next, "Failed get_multi_call"));
...
VertexPtr GenTree::get_foreach() {
  ...
  CE (!mlang_error (first_node.not_null(), "Failed to parse 'foreach' params"));

  CE (expect (tok_clpar, "')'"));

  VertexPtr second_node = get_statement();
  CE (!mlang_error (second_node.not_null(), "Failed to parse 'foreach' body"));
...
VertexPtr GenTree::get_while() {
  ...
  CE (expect (tok_oppar, "'('"));
  VertexPtr first_node = get_expression();
  CE (!mlang_error (first_node.not_null(), "Failed to parse 'while' condition"));
  first_node = conv_to <tp_bool> (first_node);
  CE (expect (tok_clpar, "')'"));

  VertexPtr second_node = get_statement();
  CE (!mlang_error (second_node.not_null(), "Failed to parse 'while' body"));
...
VertexPtr GenTree::get_if() {
  ...
  CE (expect (tok_oppar, "'('"));
  VertexPtr first_node = get_expression();
  CE (!mlang_error (first_node.not_null(), "Failed to parse 'if' condition"));
  first_node = conv_to <tp_bool> (first_node);
  CE (expect (tok_clpar, "')'"));

  VertexPtr second_node = get_statement();
  CE (!mlang_error (second_node.not_null(), "Failed to parse 'if' body"));
  ...
  if ((*cur)->type() == tok_else) {
    ...
    CE (!mlang_error (third_node.not_null(), "Failed to parse 'else' statement"));
  }
...
VertexPtr GenTree::get_for() {
  ...
  CE (!mlang_error (ok_first_next, "Failed to parse 'for' precondition"));
  ...
  CE (!mlang_error (ok_second_next, "Failed to parse 'for' action"));
  ...
  CE (!mlang_error (ok_third_next, "Failed to parse 'for' postcondition"));
  ...
  CE (!mlang_error (cmd.not_null(), "Failed to parse 'for' statement"));
...
VertexPtr GenTree::get_do() {
  ...
  CE (!mlang_error (first_node.not_null(), "Failed to parser 'do' condition"));

  CE (expect (tok_while, "'while'"));

  CE (expect (tok_oppar, "'('"));
  VertexPtr second_node = get_expression();
  CE (!mlang_error (second_node.not_null(), "Faild to parse 'do' statement"));
...
VertexPtr GenTree::get_switch() {
  ...
  CE (!mlang_error (switch_val.not_null(), "Failed to parse 'switch' expression"));
  ...
  while ((*cur)->type() != tok_clbrc) {
    ...
    if (cur_type == tok_case) {
      ...
      CE (!mlang_error (case_val.not_null(), "Failed to parse 'case' value"));
...
VertexPtr GenTree::get_function (bool anonimous_flag) {
  ...
  TokenType type = (*cur)->type();
  mlang_assert (test_expect (tok_function) || test_expect (tok_ex_function));
  next_cur();
  ...
  if (test_expect (tok_varg)) {
    ...
    CE (!mlang_error (ok_params_next, "Failed to parse function params"));
  }
  ...
  if ((*cur)->type() == tok_opbrc) {
    ...
    cmd = get_statement();
    exit_function();
    mlang_error (type != tok_ex_function, "Extern function header should not have a body");
    CE (!mlang_error (cmd.not_null(), "Failed to parse function body"));
  } else {
    CE (expect (tok_semicolon, "';'"));
  }
...
bool GenTree::check_seq_end() {
  if (!test_expect (tok_clbrc)) {
    mlang_error (0, "Failed to parse sequence");
...
bool GenTree::check_statement_end() {
  ...
  if (!test_expect (tok_semicolon)) {
    mlang_error (0, "Failed to parse statement");
...
VertexPtr GenTree::get_class() {
  ...
  CE (!mlang_error (test_expect (tok_func_name), "Class name expected"));
  ...
  CE (!mlang_error (class_body.not_null(), "Failed to parse class body"));
...
VertexPtr GenTree::get_statement() {
  ...
  switch (type) {
    ...
    case tok_opbrc:
      next_cur();
      res = get_seq();
      mlang_error (res.not_null(), "Failed to parse sequence");
      CE (check_seq_end());
      return res;
    case tok_return:
      res = get_return();
    ...
    case tok_throw: {
      ...
      next_cur();
      first_node = get_expression();
      CE (!mlang_error (first_node.not_null(), "Empty expression in throw"));
    ...
    case tok_try: {
      ...
      CE (!mlang_error (first_node.not_null(), "Cannot parse try block"));
      CE (expect (tok_catch, "'catch'"));
      CE (expect (tok_oppar, "'('"));
      CE (expect (tok_Exception, "'Exception'"));
      second_node = get_expression();
      CE (!mlang_error (second_node.not_null(), "Cannot parse catch ( ??? )"));
      CE (!mlang_error (second_node->type() == op_var, "Expected variable name in 'catch'"));
      second_node->type_help = tp_Exception;

      CE (expect (tok_clpar, "')'"));
      third_node = get_statement();
      CE (!mlang_error (third_node.not_null(), "Cannot parse catch block"));
...
VertexPtr GenTree::post_process (VertexPtr root) {
  ...
  if (root->type() == op_func_call && root->get_string() == "call_user_func_array") {
    ...
    mlang_error ((int)args.size() == 2, dl_pstr ("Call_user_func_array expected 2 arguments, got %d", (int)root->size()));
    mlang_error (args[0]->type() == op_string, "First argument of call_user_func_array must be a const string");
  ...
  if (root->type() == op_arrow) {
    ...
    mlang_error (func->type() == op_func_call, "Operator '->' expects function call as its right operand");

    string prefix = get_memfunc_prefix (func->str_val);
    if (prefix.empty()) {
      mlang_error (0, dl_pstr ("Unknown member functions [%s->%s]",
...
VertexPtr GenTree::run() {
  ...
  mlang_assert (res.is_null());
  if (cur != end) {
    fprintf (stderr, "line %d: something wrong\n", line_num);
    mlang_error (0, "Cannot compile (probably problems with brace balance)");
  }
