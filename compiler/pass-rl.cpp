...
void rl_func_call_calc (VertexPtr root, RLValueType expected_rl_type) {
  mlang_error (expected_rl_type != val_l, "Function result cannot be used as lvalue");
  switch (root->type()) {
    case op_list:
...
void rl_common_calc (VertexPtr root, RLValueType expected_rl_type) {
  mlang_assert (expected_rl_type == val_none);
  switch (root->type()) {
    case op_if:
    case op_do:
    case op_while:
  ...
  switch (OpInfo::rl (tp)) {
    case rl_set:
      switch (expected_rl_type) {
        ...
        case val_l:
          mlang_error (0, dl_pstr ("trying to use result of [%s] as lvalue", OpInfo::str (tp).c_str()));
          break;
        default:
          mlang_fail();
          break;
    ...
    case rl_index: {
      ...
      switch (expected_rl_type) {
        ...
        case val_r:
        case val_none:
          mlang_error (array->type() == op_var || array->type() == op_index || array->type() == op_func_call,
    ...
    case rl_op:
      switch (expected_rl_type) {
        case val_l:
          mlang_error (0, "Can't make result of operation to be lvalue");
          break;
        case val_r:
        case val_none:
          rl_r_calc (root, -1);
    ...
    case rl_const:
      switch (expected_rl_type) {
        case val_l:
          mlang_error (0, "Can't make const to be lvalue");
          break;
        case val_r:
        case val_none:
          break;
    ...
    case rl_var:
      switch (expected_rl_type) {
        case val_l:
          mlang_error (root->extra_type != op_ex_var_const, "Can't make const to be lvalue");
          break;
        case val_r:
        case val_none:
          break;
