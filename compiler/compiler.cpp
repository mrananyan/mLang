...
void CompilerArgs::set_functions_txt (const string &s) {
  mlang_error (functions_txt.empty(), "Several functions.txt arguments have been given to compiler");
  functions_txt = s;
}
void CompilerArgs::set_dest_dir (const string &s) {
  mlang_error (dest_dir.empty(), "Several dest_dir arguments have been given to compiler");
  dest_dir = s;
  fix_dirname (&dest_dir);
}
void CompilerArgs::set_index_path (const string &new_index_path) {
  mlang_error (index_path.empty(), "Several index arguments have been given to compiler");
  index_path = new_index_path;
}
void CompilerArgs::set_base_dir (const string &s) {
  mlang_error (base_dir.empty(), "Several base_dir arguments have been given to compiler");
  base_dir = s;
  fix_dirname (&base_dir);
}
void CompilerArgs::set_threads_number (int new_threads_number) {
  mlang_error (1 <= new_threads_number && new_threads_number <= 32, "Bad threads number");
...
class ParseF {
  public:
    ...
    template <class OutputStream> void execute (FileAndTokens file_and_tokens, OutputStream &os) {
      ...
      mlang_assert (file_and_tokens.file.not_null());
    ...
    VertexPtr on_exit_vertex (VertexPtr root, LocalT *local) {
      ...
      if (root->type() == op_require || root->type() == op_require_once) {
        ...
        for (VertexRange i = require->args(); !i.empty(); i.next()) {
          mlang_error_act ((*i)->type() == op_string, "Not a string in 'require' arguments", continue);
          ...
          if (file.not_null()) {
            ...
            *i = call;
            ...
            mlang_error (0, dl_pstr ("Cannot require [%s]\n", cur->str_val.c_str()));
          }
        }
      }
...
void function_apply_header (FunctionPtr func, VertexAdaptor <meta_op_function> header) {
  ...
  mlang_assert (root.not_null() && header.not_null());
  mlang_error_return (
    func->header.is_null(),
...
void prepare_function_misc (FunctionPtr func) {
  ...
  mlang_assert (func_root.not_null());
  ...
  for (int i = 0; i < param_n; i++) {
    if (func->varg_flag) {
      mlang_error (params[i].as <meta_op_func_param>()->var()->ref_flag == false,
    ...
    VertexPtr on_exit_vertex (VertexPtr v, LocalT *local) {
      switch (OpInfo::cnst (v->type())) {
        ...
        case cnst_not_func:
          ...
          break;
        default:
          mlang_error (0, dl_pstr ("Unknown cnst-type for [op = %d]", v->type()));
          mlang_fail();
          break;
      }
      return v;
...
class CheckFunctionCallsPass : public FunctionPassBase {
  ...
  public:
    ...
    void check_func_call (VertexPtr call) {
      mlang_assert (call->type() == op_func_call || call->type() == op_func_ptr);

      FunctionPtr f = call->get_func_id();
      mlang_assert (f.not_null());
      mlang_error_return (f->root.not_null(), dl_pstr ("Function [%s] undeclared", f->name.c_str()));
      ...
      for (int i = 0; i < call_params_n; i++) {
        if (func_params[i]->type() == op_func_param_callback) {
          ...
          for (int j = 0; j < (int)cur_params.size(); j++) {
            mlang_error (cur_params[j]->type() == op_func_param,
                       "Callback function with callback parameter");
            mlang_error (cur_params[j].as <op_func_param>()->var()->ref_flag == 0,
                       "Callback function with reference parameter");
          }
        } else {
          mlang_error (call_params[i]->type() != op_func_ptr, "Unexpected function pointer");
        }
      }
    }
  ...
  public:
    ...
    void check_uninited() {
      for (int i = 0; i < (int)uninited_vars.size(); i++) {
        ...
        mlang_warning (dl_pstr ("Variable [%s] may be used uninitialized", var->name.c_str()));
      }
    }
  ...
  public:
    ...
    VertexPtr on_enter_vertex (VertexPtr vertex, LocalT *local) {
      if (vertex->type() == op_func_name) {
        mlang_error (0, "Unexpected function name");
      }
      ...
      if (v->type() == op_func_call || v->type() == op_var ||
        ...
        if (v->rl_type == val_r) {
          ...
          if (type->ptype() == tp_Unknown && !type->use_or_false()) {
            ...
            mlang_error (0, dl_pstr ("Using Unknown type : %s", desc.c_str()));
            return true;
          }
        }
      }
...
string get_subdir (const string &file, const string &base) {
  mlang_assert (G->args().use_subdirs);
        ...
        if (file->on_disk) {
          if (file->crc64 == (unsigned long long)-1) {
            ...
            if (fscanf (old_file, "//crc64:%Lx", &old_crc) != 1) {
              mlang_warning (dl_pstr ("can't read crc64 from [%s]\n", full_file_name.c_str()));
              old_crc = -1;
            }
            fclose (old_file);
