  ...
  private:
    ...
    LValue as_lvalue (VertexPtr v) {
      ...
      if (v->type() == op_var) {
        ...
        mlang_error (0, dl_pstr ("Bug in compiler: Trying to use [%s] as lvalue", OpInfo::str (v->type()).c_str()));
        mlang_fail();
      }

      mlang_assert (value != 0);
          ...
          for (int i = -1; i < params_n; i++) {
            ...
            if (i == -1) {
              if (function->root->type_rule.not_null()) {
                mlang_error_act (function->root->type_rule->type() == op_common_type_rule, "...", continue);
                VertexAdaptor <op_common_type_rule> common_type_rule = function->root->type_rule;
                VertexPtr rule = common_type_rule->expr();
                mlang_error_act (rule->type() == op_type_rule, "...", continue);
                x = rule->type_help;
              }
            } else {
