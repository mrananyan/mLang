    ...
    *data = UBMergeData::create_from_func_ptr (v);
    ...
    if (res > 0) {
      ...
      if (supported) {
        ...
        PROF (B).start();
        mlang_warning (dl_pstr ("Dangerous undefined behaviour %s, [var = %s]",
...
void fix_ub (VertexPtr v, vector <VarPtr> *foreach_vars) {
  ...
  if (err > 0) {
    mlang_warning (dl_pstr ("Dangerous undefined behaviour %s, [foreach var = %s]",
        OpInfo::str (v->type()).c_str(), (*last_ub_error)->name.c_str()));
  }
}
